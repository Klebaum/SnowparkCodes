-- A Procedure atualmente não possui parâmetros, no entanto, é possível adicionar, para fazer isso vá nas python Worksheets cole o código do arquivo proc_snowflake.py, passe os parâmetros na função main e alteres as funções para receber esses parâmetros. Depois clique em deploy e adicione os parâmetros pela interface do Snow, depois sugiro abrir em um worksheet para ver se está tudo certo.
